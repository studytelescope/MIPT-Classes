...
class BoW(TransformerMixin):
    ...
    def fit(self, X: np.ndarray, y=None):
        ...
        text = ' '.join(list(X)).split()
        tokens_freq = Counter(text)
        self.bow = [word for word, freq in tokens_freq.most_common(self.k)]
    ...
    def _text_to_bow(self, text: str) -> np.ndarray:
        ...
        """
        text_vocab = Counter(text.split())
        result = [text_vocab[token] if token in text_vocab.keys() else 0 for token in self.bow]
...
class TfIdf(TransformerMixin):
    ...
    def fit(self, X: np.ndarray, y=None):
        ...
        if self.k is not None:
            top_words = [word for word, freq in tokens_freq.most_common(self.k)]
            tokens_freq = {word : tokens_freq[word] for word in tokens_freq if word in top_words}
        print(tokens_freq['and'])
        self.idf = {word : np.log(len(X) / tokens_freq[word]) for word in tokens_freq}
        # fit method must always return self
        return self
